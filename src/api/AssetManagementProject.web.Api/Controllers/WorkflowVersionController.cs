...
namespace AssetManagementProject.web.Api.Controllers
{
    ...
    public class WorkflowVersionController : ControllerBase
    {
        ...
        public WorkflowVersionController(WorkflowVersionService<WorkflowVersionViewModel, WorkflowVersion> workflowVersionService)
        {
        ...
        [Authorize]
        [HttpGet]
        public IEnumerable<WorkflowVersionViewModel> GetAll()
        {
            var items = _workflowVersionService.GetAll();
            return items;
        }
        ...
        {
            var item = _workflowVersionService.GetOne(id);
            if (item == null)
            {
        ...
        [HttpPost]
        public IActionResult Create([FromBody] WorkflowVersionViewModel workflowVersion)
        {
            if (workflowVersion == null)
                return BadRequest();

            var id = _workflowVersionService.Add(workflowVersion);
        ...
        [Authorize(Policy = AuthorizationRolesConstants.Administrator)]
        [HttpPut("{id}")]
        public IActionResult Update(int id, [FromBody] WorkflowVersionViewModel workflowVersion)
        {
            if (workflowVersion == null || workflowVersion.Id != id)
                return BadRequest();

            int retVal = _workflowVersionService.Update(workflowVersion);
            if (retVal == 0)
        ...
        {
            int retVal = _workflowVersionService.Remove(id);
            if (retVal == 0)
