...
namespace AssetManagementProject.web.STS.Identity.Helpers
{
    ...
    {
        public static string GetEmailAddressFromClaims(List<Claim> claims, string provider)
        {
            if (claims.Count() > 0)
            {
                foreach (var claim in claims)
                {
                    ...
                    try
                    {
                        var emailAddress = new MailAddress(claim.Value);
                    }
                    catch
                    {
                        isValidEmail = false;
                    ...
                    if (isValidEmail) return claim.Value;
                }
            }
